...
//---------------------------------------------------------------------------------------------------------------
//DO NOT CHANGE THIS PART BELOW (EXCEPT TIMING FIX FOR CORRECTNESS)
...
void run_test(const char* name, sort_type* arr, int n) {
    ...
    // CRITICAL FIX: Use omp_get_wtime() instead of clock() for accurate timing
    ...
    if (verify_sorted(arr, n)) {
        double time_taken = end - start;  // Already in seconds
        printf("RESULT: PASSED [%.6f sec]\n", time_taken);
    } else {
        printf("RESULT: FAILED!\n");
    }
...
void run_gb_test(int gb) {
    ...
    // 4. Run Sort - CRITICAL FIX: Use omp_get_wtime() instead of clock()
    printf("[INFO] Sorting...\n");
    double start = omp_get_wtime();
...
int main() {
    ...
    printf("Insertion Sort Threshold: %d\n", INSERTION_SORT_THRESHOLD);
    ...
    // HIGH RAM USAGE
    run_gb_test(4);  // Requires ~8GB RAM
    // run_gb_test(8);  // Requires ~16GB RAM - Comment out if low RAM
    //run_gb_test(10); // Requires ~20GB RAM
